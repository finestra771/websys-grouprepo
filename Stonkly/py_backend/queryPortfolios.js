...
const margin = { top: 50, right: 120, bottom: 50, left: 60 }; // extra right space for legend
...
function drawCmpChart(data) {
  ...
  g.append("text")
    ...
    .style("fill", "#B87432") // Adjust color to match background
    .style("font-family", "BMDOHYEON_TTF") // Adjust font style
  ...
  zoomLayer.selectAll(".benchmark-circle").data(data).enter()
    ...
    `)
      ...
      d3.select(event.currentTarget).attr("r", 6); // optional: enlarge circle on hover
    })
    .on("mouseout", (event) => {
      tooltip.transition().duration(200).style("opacity", 0);
      d3.select(event.currentTarget).attr("r", 4); // reset circle size
    });

  // Circles on portfolio line
  zoomLayer.selectAll(".portfolio-circle").data(data).enter()
    ...
    .on("mouseover", (event, d) => {
      tooltip.transition().duration(200).style("opacity", 1);
      tooltip.html(`<strong>${d.date.toDateString()}</strong><br>Portfolio: ${d.cumulative_portfolio}<br>Benchmark: ${d.cumulative_benchmark}<br>Tariff stories: ${d.tariff_story_count}`)
  ...
  let bars = zoomLayer.selectAll(".bars").data(data).enter()
    ...
    .on("mouseover", (event, d) => {
      tooltip.transition().duration(200).style("opacity", 1);
      tooltip.html(`<strong>${d.date.toDateString()}</strong><br>Tariff stories: ${d.tariff_story_count}`)
        .style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 28) + "px");
  ...
  // Zoom only X
  const zoom = d3.zoom()
    .scaleExtent([1, 20]) // min 1 (original size), max 20× zoom
